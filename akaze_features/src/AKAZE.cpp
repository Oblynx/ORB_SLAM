...
namespace akaze_features
{
  ...
  int AKAZE::Create_Nonlinear_Scale_Space(const cv::Mat& img) {
    ...
    if (evolution_.size() == 0) {
      ...
      std::cerr << "Firstly you need to call AKAZE::Allocate_Memory_Evolution()" << std::endl;
      return -1;
    }
    ...
    for (size_t i = 1; i < evolution_.size(); i++) {
      ...
      switch (options_.diffusivity) {
        ...
        case CHARBONNIER:
          ...
          break;
        default:
          std::cerr << "Diffusivity: " << options_.diffusivity << " is not supported" << std::endl;
      }
  ...
  void AKAZE::Feature_Detection(std::vector<cv::KeyPoint>& kpts) {
    ...
    std::vector<cv::KeyPoint>().swap(kpts);
  ...
  void AKAZE::Compute_Determinant_Hessian_Response() {
    ...
    for (size_t i = 0; i < evolution_.size(); i++) {
      ...
      if (options_.verbosity == true)
        std::cout << "Computing detector response. Determinant of Hessian. Evolution time: " << evolution_[i].etime << std::endl;
