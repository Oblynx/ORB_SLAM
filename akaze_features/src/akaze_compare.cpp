...
int main(int argc, char *argv[]) {
  ...
  // ORB variables
  std::vector<cv::KeyPoint> kpts1_orb, kpts2_orb;
  std::vector<cv::Point2f> matches_orb, inliers_orb;
  std::vector<std::vector<cv::DMatch> > dmatches_orb;
  ...
  // BRISK variables
  std::vector<cv::KeyPoint> kpts1_brisk, kpts2_brisk;
  std::vector<cv::Point2f> matches_brisk, inliers_brisk;
  std::vector<std::vector<cv::DMatch> > dmatches_brisk;
  ...
  // AKAZE variables
  std::vector<cv::KeyPoint> kpts1_akaze, kpts2_akaze;
  std::vector<cv::Point2f> matches_akaze, inliers_akaze;
  std::vector<std::vector<cv::DMatch> > dmatches_akaze;
  ...
  if (img1.data == NULL) {
    std::cerr << "Error loading image: " << img_path1 << std::endl;
    return -1;
  }
  ...
  if (img2.data == NULL) {
    std::cerr << "Error loading image: " << img_path2 << std::endl;
    return -1;
  }
  ...
  akaze_features::matches2points_nndr(kpts1_orb,kpts2_orb,dmatches_orb,matches_orb,DRATIO);

  if (use_ransac == false)
    akaze_features::compute_inliers_homography(matches_orb,inliers_orb, HG, MIN_H_ERROR);
  else
    akaze_features::compute_inliers_ransac(matches_orb, inliers_orb, MIN_H_ERROR, false);
  ...
  cv::cvtColor(img1, img1_rgb_orb, cv::COLOR_GRAY2BGR);
  cv::cvtColor(img2, img2_rgb_orb, cv::COLOR_GRAY2BGR);

  akaze_features::draw_keypoints(img1_rgb_orb, kpts1_orb);
  akaze_features::draw_keypoints(img2_rgb_orb, kpts2_orb);
  akaze_features::draw_inliers(img1_rgb_orb, img2_rgb_orb, img_com_orb, inliers_orb, 0);
  ...
  std::cout << "ORB Features Extraction Time (ms): " << torb << std::endl;
  std::cout << std::endl;
  ...
  akaze_features::matches2points_nndr(kpts1_brisk, kpts2_brisk, dmatches_brisk, matches_brisk, DRATIO);

  if (use_ransac == false)
    akaze_features::compute_inliers_homography(matches_brisk, inliers_brisk, HG, MIN_H_ERROR);
  else
    akaze_features::compute_inliers_ransac(matches_brisk, inliers_brisk, MIN_H_ERROR, false);
  ...
  cv::cvtColor(img1, img1_rgb_brisk, cv::COLOR_GRAY2BGR);
  cv::cvtColor(img2, img2_rgb_brisk, cv::COLOR_GRAY2BGR);

  akaze_features::draw_keypoints(img1_rgb_brisk, kpts1_brisk);
  akaze_features::draw_keypoints(img2_rgb_brisk, kpts2_brisk);
  akaze_features::draw_inliers(img1_rgb_brisk, img2_rgb_brisk, img_com_brisk, inliers_brisk, 1);

  std::cout << "BRISK Results" << std::endl;
  std::cout << "**************************************" << std::endl;
  std::cout << "Number of Keypoints Image 1: " << nkpts1_brisk << std::endl;
  std::cout << "Number of Keypoints Image 2: " << nkpts2_brisk << std::endl;
  std::cout << "Number of Matches: " << nmatches_brisk << std::endl;
  std::cout << "Number of Inliers: " << ninliers_brisk << std::endl;
  std::cout << "Number of Outliers: " << noutliers_brisk << std::endl;
  std::cout << "Inliers Ratio: " << ratio_brisk << std::endl;
  std::cout << "BRISK Features Extraction Time (ms): " << tbrisk << std::endl;
  std::cout << std::endl;
  ...
  akaze_features::matches2points_nndr(kpts1_akaze,kpts2_akaze,dmatches_akaze,matches_akaze,DRATIO);

  if (use_ransac == false)
    akaze_features::compute_inliers_homography(matches_akaze,inliers_akaze,HG,MIN_H_ERROR);
  else
    akaze_features::compute_inliers_ransac(matches_akaze,inliers_akaze,MIN_H_ERROR,false);
  ...
  cv::cvtColor(img1,img1_rgb_akaze, cv::COLOR_GRAY2BGR);
  cv::cvtColor(img2,img2_rgb_akaze, cv::COLOR_GRAY2BGR);

  akaze_features::draw_keypoints(img1_rgb_akaze, kpts1_akaze);
  akaze_features::draw_keypoints(img2_rgb_akaze, kpts2_akaze);
  akaze_features::draw_inliers(img1_rgb_akaze, img2_rgb_akaze, img_com_akaze, inliers_akaze, 2);

  std::cout << "A-KAZE Results" << std::endl;
  std::cout << "**************************************" << std::endl;
  std::cout << "Number of Keypoints Image 1: " << nkpts1_akaze << std::endl;
  std::cout << "Number of Keypoints Image 2: " << nkpts2_akaze << std::endl;
  std::cout << "Number of Matches: " << nmatches_akaze << std::endl;
  std::cout << "Number of Inliers: " << ninliers_akaze << std::endl;
  std::cout << "Number of Outliers: " << noutliers_akaze << std::endl;
  std::cout << "Inliers Ratio: " << ratio_akaze << std::endl;
...
int parse_input_options(akaze_features::AKAZEOptions& options,
  ...
  if (argc == 1) {
    akaze_features::show_input_options_help(2);
    return -1;
  }
