...
#include "akaze_features/utils.h"


// System
#include <fstream>

namespace akaze_features
{
...
/* ************************************************************************* */
int save_keypoints(const std::string& outFile, const std::vector<cv::KeyPoint>& kpts,
...
void show_input_options_help(int example) {
  ...
  if (example == 0) {
    std::cout << "./akaze_features img.jpg [options]" << std::endl;
  }
  else if (example == 1) {
    std::cout << "./akaze_match img1.jpg img2.pgm [homography.txt] [options]" << std::endl;
  }
  else if (example == 2) {
    std::cout << "./akaze_compare img1.jpg img2.pgm [homography.txt] [options]" << std::endl;
  }
  
  std::cout << std::endl;
  cout_help() << "homography.txt is optional. If the txt file is not" 
    << " provided a planar homography will be estimated " 
    << "using RANSAC" << std::endl;

  std::cout << std::endl;
  cout_help() << "Options below are not mandatory. Unless specified, default arguments are used." << std::endl << std::endl;  

  // Justify on the left
  std::cout << std::left;

  // Generalities
  cout_help() << "--help" << "Show the command line options" << std::endl;
  cout_help() << "--verbose " << "Verbosity is required" << std::endl;
  cout_help() << std::endl;

  // Scale-space parameters
  cout_help() << "--soffset" << "Base scale offset (sigma units)" << std::endl;
  cout_help() << "--omax" << "Maximum octave of image evolution" << std::endl;
  cout_help() << "--nsublevels" << "Number of sublevels per octave" << std::endl;
  cout_help() << "--diffusivity" << "Diffusivity function. Possible values:" << std::endl;
  cout_help() << " " << "0 -> Perona-Malik, g1 = exp(-|dL|^2/k^2)" << std::endl;
  cout_help() << " " << "1 -> Perona-Malik, g2 = 1 / (1 + dL^2 / k^2)" << std::endl;
  cout_help() << " " << "2 -> Weickert diffusivity" << std::endl;
  cout_help() << " " << "3 -> Charbonnier diffusivity" << std::endl;
  cout_help() << std::endl;

  // Feature detection parameters.
  cout_help() << "--dthreshold" << "Feature detector threshold response for keypoints" << std::endl;
  cout_help() << " " << "(0.001 can be a good value)" << std::endl;
  cout_help() << std::endl;
  cout_help() << std::endl;

  // Descriptor parameters.
  cout_help() << "--descriptor" << "Descriptor Type. Possible values:" << std::endl;
  cout_help() << " " << "0 -> SURF_UPRIGHT" << std::endl;
  cout_help() << " " << "1 -> SURF" << std::endl;
  cout_help() << " " << "2 -> M-SURF_UPRIGHT," << std::endl;
  cout_help() << " " << "3 -> M-SURF" << std::endl;
  cout_help() << " " << "4 -> M-LDB_UPRIGHT" << std::endl;
  cout_help() << " " << "5 -> M-LDB" << std::endl;
  cout_help() << std::endl;

  cout_help() << "--descriptor_channels " << "Descriptor Channels for M-LDB. Valid values: " << std::endl;
  cout_help() << " " << "1 -> intensity" << std::endl;
  cout_help() << " " << "2 -> intensity + gradient magnitude" << std::endl;
  cout_help() << " " << "3 -> intensity + X and Y gradients" <<std::endl;
  cout_help() << std::endl;

  cout_help() << "--descriptor_size" << "Descriptor size for M-LDB in bits." << std::endl;
  cout_help() << " " << "0: means the full length descriptor (486)!!" << std::endl;
  cout_help() << std::endl;

  // Save results?
  cout_help() << "--show_results" << "Possible values below:" << std::endl;
  cout_help() << " " << "1 -> show detection results." << std::endl;
  cout_help() << " " << "0 -> don't show detection results" << std::endl;
  cout_help() << std::endl;
}
